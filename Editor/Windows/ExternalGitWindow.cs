...
public class ExternalGitWindow : EditorWindow
{
        ...
        using (new EditorGUILayout.VerticalScope())
        {
            ...
            using (new EditorGUILayout.HorizontalScope(EditorStyles.helpBox))
            {
                EditorUtils.IconButton(() =>
                {
                    UpdateBranches();
                    RunFullRecompute();
                }, AwesomeIcons.Refresh, Tools.Editor.EditorUtils.IconStyle.Solid, "Обновить ветки и статус зависимостей", 16, GUILayout.Width(50), GUILayout.Height(25));
                ...
                }, AwesomeIcons.Folder, EditorUtils.IconStyle.Solid, "Открыть в проводнике", 16, GUILayout.Width(50), GUILayout.Height(25));
                GUILayout.Space(10);
                // Open Git server browser in a new docked tab next to this window
                EditorUtils.IconButton(() =>
                {
                    var wnd = GetWindow<GitServerBrowserWindow>("Git Server Browser", true, typeof(ExternalGitWindow));
                    wnd.Show();
                }, AwesomeIcons.Git, EditorUtils.IconStyle.Brand, "Открыть браузер Git-сервера", 16, GUILayout.Width(50), GUILayout.Height(25));
    ...
    private void DrawDependenciesSection()
    {
        ...
        using (new EditorGUILayout.VerticalScope("box"))
        {
            ...
            {
                EditorGUILayout.HelpBox("Зависимости не заданы. Они будут добавляться сюда автоматически после клонирования через это окно.", MessageType.Info);
                return;
            }
            ...
            if (_hasOutdatedDeps)
            {
                ...
                EditorGUILayout.HelpBox("Есть не обновлённые зависимости. Рекомендуется обновить их до последних коммитов.", MessageType.Warning);
                using (new EditorGUILayout.HorizontalScope())
                {
                    GUILayout.FlexibleSpace();
    ...
    private void DrawDependencyRow(string url, string libsRoot)
    {
        ...
        using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (!info.ok)
                {
                    GUILayout.Label("Папка без .git (можно добавить в Git UI)", EditorStyles.miniLabel, GUILayout.Width(280));
                ...
                switch (info.state)
                {
                    ...
                    case GitRepoUtil.RepoUpdateState.Behind:
                        ...
                        GUILayout.Label(string.IsNullOrEmpty(info.details) ? "Есть обновления" : $"Есть обновления ({info.details})", EditorStyles.miniLabel, GUILayout.Width(220));
                    ...
                    case GitRepoUtil.RepoUpdateState.Ahead:
                        GUILayout.Label(string.IsNullOrEmpty(info.details) ? "Локальные изменения" : $"Локальные изменения ({info.details})", EditorStyles.miniLabel, GUILayout.Width(240));
                        break;
                    case GitRepoUtil.RepoUpdateState.Diverged:
                        _hasOutdatedDeps = true;
                        GUILayout.Label("Расхождение с удалённой веткой", EditorStyles.miniLabel, GUILayout.Width(240));
                        if (GUILayout.Button("Обновить", GUILayout.Width(120)))
                        {
                            UpdateDependency(info.targetPath);
                        }
                        break;
                    default:
                        GUILayout.Label("Не удалось определить состояние", EditorStyles.miniLabel, GUILayout.Width(240));
                        break;
                }
                ...
                EditorUtils.IconButton(() =>
                {
                    ...
                    if (Event.current != null && (Event.current.button == 1 || Event.current.type == EventType.ContextClick))
                    {
                ...
                }, GetExternalClientIcon(), EditorUtils.IconStyle.Brand, "Открыть в Git UI (ПКМ — выбрать клиент)", 16, GUILayout.Width(50), GUILayout.Height(25));
                
                EditorUtils.IconButton(() =>
                {
                    if (!string.IsNullOrEmpty(info.targetPath)) GitPathsUtil.PingPath(info.targetPath);
                }, AwesomeIcons.Eye, EditorUtils.IconStyle.Solid, "Пинговать путь", 16, GUILayout.Width(50), GUILayout.Height(25));
                        ...
                        if (newIdx != idx && newIdx >= 0 && newIdx < combined.Count)
                        {
                            ...
                            if (!info.hasLocalChanges)
                            {
                                if (selection.StartsWith("origin/", StringComparison.Ordinal))
                                {
                                    ...
                                    {
                                        UnityEngine.Debug.Log($"[Git] Created tracking branch {localName} for {selection} in {info.targetPath}");
                                        AssetDatabase.Refresh();
                                        RunFullRecompute();
                                    }
                                    else
                                    {
                                        EditorUtility.DisplayDialog("Смена ветки", "Не удалось создать локальную ветку для удалённой.", "OK");
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        UnityEngine.Debug.Log($"[Git] Switched {info.targetPath} to {selection}");
                                        AssetDatabase.Refresh();
                                        RunFullRecompute();
                                    }
                                    else
                                    {
                                        EditorUtility.DisplayDialog("Смена ветки", "Не удалось переключить ветку. Убедитесь, что нет локальных изменений.", "OK");
                                    }
                                }
                            }
                            else
                            {
                                EditorUtility.DisplayDialog("Смена ветки", "Нельзя переключить ветку: есть локальные изменения.", "OK");
                            }
                        }
                    }
    ...
    private enum LocalExternalGitClient { GitHubDesktop, SourceTree, VSCode }
    ...
    private bool LocalGitUiOpen(LocalExternalGitClient client, string repoPath)
    {
        try
        {
            ...
            bool isRepo = System.IO.Directory.Exists(System.IO.Path.Combine(repoPath, ".git"));
                ...
                case LocalExternalGitClient.SourceTree:
                    {
                        ...
                        {
                            System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "SourceTree", "SourceTree.exe"),
                            System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86), "Atlassian", "SourceTree", "SourceTree.exe"),
                            System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Atlassian", "SourceTree", "SourceTree.exe")
                        };
                ...
                case LocalExternalGitClient.VSCode:
                    {
                        ...
                        foreach (var exe in exeCandidates)
                        {
                            if (System.IO.File.Exists(exe) && LocalTryStartExe(exe, $"-n \"{repoPath}\"")) return true;
                        }
    ...
    private void RecomputeDependenciesState(bool lightweight)
    {
        ...
        foreach (var url in _requirements.urls)
        {
            ...
            if (info.ok)
            {
                ...
                if (info.state == GitRepoUtil.RepoUpdateState.Behind || info.state == GitRepoUtil.RepoUpdateState.Diverged)
                    _hasOutdatedDeps = true;
            }
            else
            {
    ...
    private void UpdateDependency(string repoDir)
    {
        ...
        {
            UnityEngine.Debug.Log($"[Git] Updated dependency: {repoDir}\n{output}");
        }
        else
        {
            UnityEngine.Debug.LogError($"[Git] Failed to update dependency: {repoDir}\n{output}");
        }
        AssetDatabase.Refresh();
        RefreshStatus();
    }
        ...
        foreach (var url in _requirements.urls)
        {
            ...
            try
            {
                if (Directory.Exists(target))
                {
                    ...
                    {
                        UnityEngine.Debug.LogWarning($"[Git] Пропуск: {target} существует и не пуст, но не git.");
                        errors++;
                        continue;
                    }
                    ...
                    {
                        UnityEngine.Debug.Log($"[Git] clone into existing empty:\n{out1}");
                        installed++;
                    }
                    else
                    {
                        UnityEngine.Debug.LogError($"[Git] Ошибка клонирования в {target}");
                        errors++;
                    }
                }
                else
                {
                    ...
                    if (MapTiles.Editor.Git.GitCommand.Run($"clone {url} \"{target}\"", workDir, out var out2))
                    {
                        UnityEngine.Debug.Log($"[Git] clone into new folder:\n{out2}");
                        installed++;
                    }
                    else
                    {
            ...
            {
                UnityEngine.Debug.LogError("[Git] Ошибка: " + ex.Message);
                errors++;
            }
        }
    ...
    private void UpdateAllOutdated()
    {
        ...
        var toUpdate = _depInfoByUrl.Values
            .Where(d => d.ok && (d.state == GitRepoUtil.RepoUpdateState.Behind || d.state == GitRepoUtil.RepoUpdateState.Diverged))
        ...
        foreach (var repoDir in toUpdate)
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"[Git] Failed to update dependency: {repoDir}\n{output}");
                fail++;
            }
        }
        AssetDatabase.Refresh();
    ...
    private void InstallDependency(string url, string libsRoot)
    {
        ...
        {
            UnityEngine.Debug.LogError("[Git] Ошибка установки зависимости: " + ex.Message);
        }
        RefreshStatus();
    }
    ...
    private void TryDeleteDependency(string url, DepInfo info)
    {
        ...
        {
            UnityEngine.Debug.LogError("[Git] Ошибка удаления: " + ex.Message);
        }
